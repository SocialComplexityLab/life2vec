...
class EmmSubPopulation(Population):
    ...
    def target(self) -> pd.DataFrame:
        ...
        result = data.loc[lambda x: (x.PERSON_ID.isin(population_ids)) & \
                                    ...
                                    (x.EVENT_CAUSE_FINAL.isin(["Udvandret"]))] \
        ...
        ##### UNLABALED
        self.unlabeled_ids = temp_result_1.index.to_numpy().tolist()
    ...
    def data_split(self) -> DataSplit:
        ...
        ## print the unlabeled number
        train_unlabeled = np.intersect1d(train_ids, self.unlabeled_ids)
        self.ratio_unlabeled = float(train_unlabeled.shape[0])/ float(train_ids.shape[0])
        logger.info("Fraction of unlabeled samples in TRAIN: %.3f" %self.ratio_unlabeled)
